"""Contract tests for every legacy field, without contacting account servers."""
import copy
import unittest
from unittest.mock import patch
from jsonschema import Draft202012Validator
from editor_legacy import legacy_to_operations, SUPPORTED_FIELDS, CREDENTIAL_FIELDS, SIMPLE_FLAGS, MAP_FLAGS, _catalog


# Fixed snapshot of all 115 distinct keys read by the old main.py /edit parser.
...
class LegacyContractTests(unittest.TestCase):
    def test_every_old_parser_field_has_a_schema_valid_translation(self):
        ...
        for field in sorted(ORIGINAL_FIELDS):
            with self.subTest(field=field):
                operations = legacy_to_operations({field: cases[field]})
    ...
    def test_zero_values_are_never_lost(self):
        operations = legacy_to_operations({'catfood': 0, 'xp': 0, 'catamins_b': 0, 'playtime': 0, 'rare_gatya_seed': 0})
    ...
    def test_false_boolean_flags_do_not_invoke_edits(self):
        ...
        self.assertEqual(legacy_to_operations({field: False for field in fields}), [])
        operations = legacy_to_operations({'outbreaks': {'chapters': [0], 'cleared': False}, 'cat_shrine': {'visible': False}})
    ...
    def test_scalar_bools_floats_strings_and_overflows_are_rejected(self):
        for field in ('xp', 'catfood', 'inquiry_code', 'password_refresh_token', 'unlocked_slots', 'restart_pack'):
            with self.subTest(field=field), self.assertRaises(ValueError):
                legacy_to_operations({field: True})
        for value in (True, 1.0, '5', -1, 2147483648, None):
            with self.subTest(value=value), self.assertRaises(ValueError):
                legacy_to_operations({'xp': value})
        for value in (0, 1, 'false', None):
            with self.subTest(value=value), self.assertRaises(ValueError):
                legacy_to_operations({'unlock_cats': value})
    ...
    def test_safety_is_off_by_default_and_only_added_where_supported(self):
        ...
        for safety in (False, True):
            operations = legacy_to_operations({**payload, 'enable_safety': safety})
            catalog = _catalog()
            for operation in operations:
        ...
        self.assertIs(legacy_to_operations({'xp': 5})[0]['args']['respect_maxima'], False)
        seed = legacy_to_operations({'rare_gatya_seed': 4000000000})[0]
        self.assertEqual(seed['args']['value'], 4000000000)

    def test_catamin_aliases_and_catseye_names_preserve_unspecified_indexes(self):
        self.assertEqual(legacy_to_operations({'catamins_c': 7})[0]['args']['values'], {'2': 7})
        self.assertEqual(legacy_to_operations({'catseyes': {'rare': 7, 'uber': 0}})[0]['args']['values'], {'1': 7, '3': 0})
        with self.assertRaises(ValueError):
            legacy_to_operations({'catseyes': {'ex': 1, 'special': 2}})
        with self.assertRaises(ValueError):
            legacy_to_operations({'catseyes': {'superrare_typo': 2}})
        self.assertEqual(legacy_to_operations({'catfruit': [0, 9]})[0]['args']['values'], [0, 9])

    def test_aliases_do_not_override_conflicting_values(self):
        with self.assertRaises(ValueError):
            legacy_to_operations({'true_form_all': False, 'max_cat_evolutions': True})
        with self.assertRaises(ValueError):
            legacy_to_operations({'cat_forms': {'0': 1}, 'cat_evolutions': {'0': 3}})
        operations = legacy_to_operations({'claim_rewards': True, 'claim_all_rewards': True})
    ...
    def test_ambiguous_old_menu_flags_request_explicit_arguments(self):
        for field in ('cat_storage', 'cat_shrine', 'ototo_cat_cannon', 'event_tickets', 'behemoth_stones', 'gamatoto_helpers'):
            with self.subTest(field=field), self.assertRaises(ValueError):
                legacy_to_operations({field: True})
        self.assertEqual(legacy_to_operations({'behemoth_stones': {'item_ids': {'166': 5}}})[0]['action'], 'items.evolve_by_id')
    ...
    def test_clear_all_stages_includes_every_stage_family(self):
        operations = legacy_to_operations({'clear_all_stages': True})
        ...
        selected = legacy_to_operations({'clear_all_stages': {'scopes': ['story', 'aku']}})
    ...
    def test_treasure_edits_never_add_score_edits(self):
        operations = legacy_to_operations({'max_treasures': True, 'stage_treasures': [{'chapter': 0, 'stage': 1, 'treasure': 0}]})
    ...
    def test_aku_single_stage_keeps_map_and_crown(self):
        operation = legacy_to_operations({'clear_stages': [{'chapter': 9, 'stage': 2, 'map': 1, 'star': 0, 'clears': 0}]})[0]
    ...
    def test_storage_operations_are_explicit(self):
        operation = legacy_to_operations({'cat_storage': {'operation': 'clear', 'confirm': True}})[0]
        self.assertEqual(operation, {'action': 'cats.storage.clear', 'args': {'confirm': True}})
        with self.assertRaises(ValueError):
            legacy_to_operations({'cat_storage': {'operation': 'clear'}})
    ...
    def test_translated_partial_vectors_change_only_requested_entries(self):
        ...
        operations = legacy_to_operations({'catamins_b': 0, 'catseyes': {'rare': 0}})
        catalog = _catalog()
        for operation in operations:
    ...
    def test_legacy_treasure_raw_slot_is_preserved(self):
        from types import SimpleNamespace
        from bcsfe import core
        sf = SimpleNamespace(story=core.StoryChapters.init())
        operation = legacy_to_operations({'stage_treasures': [{'chapter': 0, 'stage': 0, 'treasure': 2}]})[0]
